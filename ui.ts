...
<head>
  ...
  <script>
    // --- 2D LIVE UPDATE LOGIC ---
    function start2DLive() {
        ...
        if(!liveNum) return; // Not on 2D page

        setInterval(async () => {
            try {
                const res = await fetch('/api/2d-proxy'); // Fetch from our backend proxy
                const data = await res.json();
                
                if(data.live) {
                    liveNum.innerText = data.live.twod;
                    liveSet.innerText = data.live.set;
                    liveVal.innerText = data.live.value;
                    liveTime.innerText = data.live.time;
                    
                    // Blinking Effect
                    statusDot.classList.add('text-green-500');
                    statusDot.classList.remove('text-red-500');
                }
            } catch (e) {
                statusDot.classList.add('text-red-500');
                statusDot.classList.remove('text-green-500');
            }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // ... (Other existing scripts: Slider, Loader, etc.) ...
    ...
    // ... (Existing Copy, Filter, Modal functions - KEEP THEM) ...
    function copyToClipboard(text, btnId = 'copyBtn') { navigator.clipboard.writeText(text).then(() => { const btn = document.getElementById(btnId); if(btn) { const originalText = btn.innerText; btn.innerText = '✅ Copied!'; btn.classList.remove('bg-blue-600'); btn.classList.add('bg-green-600'); setTimeout(() => { btn.innerText = "Copy Code"; btn.classList.remove('bg-green-600'); btn.classList.add('bg-blue-600'); }, 2000); } }); }
    function copyPurchasedCode() { const btn = document.getElementById('copyBtnModal'); const rawCode = btn.getAttribute('data-code'); copyToClipboard(rawCode, 'copyBtnModal'); }
    function filterProducts() { const input = document.getElementById('searchInput'); const filter = input.value.toLowerCase(); const nodes = document.querySelectorAll('.product-card'); nodes.forEach(node => { const name = node.dataset.name.toLowerCase(); node.style.display = name.includes(filter) ? "flex" : "none"; }); }
    function disableProductCard(id) { const btn = document.getElementById("btn-" + id); const badge = document.getElementById("badge-" + id); if(btn) { btn.disabled = true; btn.classList.remove("bg-blue-600", "hover:bg-blue-500"); btn.classList.add("bg-slate-700", "cursor-not-allowed"); btn.innerText = "🚫 Out of Stock"; btn.removeAttribute("onclick"); } if(badge) { badge.classList.remove("bg-green-500/20", "text-green-400"); badge.classList.add("bg-red-500/20", "text-red-400"); } }
    let selectedProductId = null;
    function confirmBuy(id, name, price) { selectedProductId = id; document.getElementById('confirmName').innerText = name; document.getElementById('confirmPrice').innerText = price + " Ks"; document.getElementById('confirmModal').classList.remove('hidden'); }
    ...
    function showErrorModal(msg, isBalanceError = false) { document.getElementById('errorMessage').innerText = msg; const btnContainer = document.getElementById('errorBtnContainer'); if(isBalanceError) { btnContainer.innerHTML = \`<div class="flex gap-3 w-full"><button onclick="closeErrorModal()" class="bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 px-4 rounded-xl transition">✕</button><a href="/deposit" class="flex-1 bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-xl transition shadow-lg text-center flex items-center justify-center">Top Up Now</a></div>\`; } else { btnContainer.innerHTML = \`<button onclick="closeErrorModal()" class="w-full bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 rounded-xl transition">Close</button>\`; } document.getElementById('errorModal').classList.remove('hidden'); }
    async function processPurchase() { if(!selectedProductId) return; const confirmBtn = document.getElementById('confirmBtnAction'); const originalText = confirmBtn.innerText; confirmBtn.innerHTML = '<div class="loader-sm mx-auto"></div>'; confirmBtn.disabled = true; try { const res = await fetch("/buy", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: selectedProductId }) }); const data = await res.json(); closeConfirmModal(); if(data.success) { document.getElementById('purchasedCode').innerText = data.code; const copyBtn = document.getElementById('copyBtnModal'); copyBtn.setAttribute('data-code', data.rawCode); document.getElementById('successModal').classList.remove('hidden'); document.querySelectorAll('.balance-display').forEach(el => el.innerText = data.newBalance.toLocaleString() + " Ks"); } else { const isBalanceError = data.message === "Insufficient Balance"; showErrorModal(data.message || "Purchase Failed", isBalanceError); } } catch (e) { showErrorModal("Connection Error"); } finally { confirmBtn.innerText = originalText; confirmBtn.disabled = false; } }
    function selectAvatar(avatar) { document.getElementById('selectedAvatarInput').value = avatar; document.querySelectorAll('.avatar-option').forEach(el => el.classList.remove('ring-4', 'ring-blue-500')); document.getElementById('av-' + avatar).classList.add('ring-4', 'ring-blue-500'); }
    function toggleProductInputs() { const type = document.getElementById('productType').value; document.getElementById('input-manual').style.display = 'none'; document.getElementById('input-api').style.display = 'none'; document.getElementById('input-shared').style.display = 'none'; if (type === 'manual') document.getElementById('input-manual').style.display = 'block'; else if (type === 'api') document.getElementById('input-api').style.display = 'block'; else if (type === 'shared') document.getElementById('input-shared').style.display = 'block'; }
  </script>
  <style>
...
<body class="min-h-screen flex flex-col relative pb-24">
        ...
        ${user ? `
          <div class="flex items-center gap-3">
              ...
              <a href="/profile" class="hidden md:block relative">
                <div class="w-9 h-9 rounded-md bg-slate-700 flex items-center justify-center text-xl border border-slate-500 shadow-sm hover:ring-2 ring-blue-500 transition">${user.avatar || "😎"}</div>
              </a>
  ...
  <div class="md:hidden fixed bottom-0 left-0 w-full z-[50] bottom-nav-container">
      <div class="flex justify-around items-center py-3">
          ...
          <a href="/2d" id="nav-2d" class="flex flex-col items-center gap-1 text-slate-500 hover:text-blue-400 transition w-full">
              <div class="relative">
                 <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                 <span id="status-dot" class="absolute -top-1 -right-1 w-2 h-2 bg-slate-500 rounded-full animate-pulse"></span>
              </div>
              <span class="text-[10px] font-medium">2D Live</span>
          </a>
            ...
            <div class="p-6 text-center">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div class="bg-slate-800 p-3 rounded-xl border border-slate-700">
                        <div class="text-slate-400 mb-1">SET</div>
                        <div class="text-xl font-bold text-blue-400 font-mono" id="live-set">0.00</div>
                    </div>
                    <div class="bg-slate-800 p-3 rounded-xl border border-slate-700">
                        <div class="text-slate-400 mb-1">VALUE</div>
                        <div class="text-xl font-bold text-green-400 font-mono" id="live-val">0.00</div>
                    </div>
                </div>
            </div>
        </div>
...
// (Other components: ImageSlider, AuthForm, MaintenancePage, ProductCard, HistoryTable, ProfilePage, TransferPage, AdminUserTable, AdminSalesTable - SAME AS BEFORE)
...
export const AuthForm = (type: "Login" | "Register", error?: string) => `<div class="max-w-md mx-auto glass p-8 rounded-2xl shadow-2xl"><h2 class="text-3xl font-bold text-center mb-6 text-white">${type}</h2>${error ? `<div class="bg-red-500/20 border border-red-500 text-red-200 p-3 rounded mb-4 text-center">${error}</div>` : ''}<form method="POST" class="space-y-4"><div><label class="block text-sm font-medium text-slate-400 mb-1">Username</label><input type="text" name="username" required class="w-full bg-slate-800 border border-slate-600 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none text-white"></div><div><label class="block text-sm font-medium text-slate-400 mb-1">Password</label><input type="password" name="password" required class="w-full bg-slate-800 border border-slate-600 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none text-white"></div>${type === 'Login' ? `<label class="flex items-center gap-2 text-slate-400 text-sm cursor-pointer"><input type="checkbox" name="remember" class="rounded bg-slate-800 border-slate-600 text-blue-600 focus:ring-blue-500">Remember me</label>` : ''}<button class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-lg transition shadow-lg shadow-blue-500/30">${type}</button></form><p class="mt-4 text-center text-slate-400 text-sm">${type === 'Login' ? 'Don\'t have an account? <a href="/register" class="text-blue-400">Register</a>' : 'Already have an account? <a href="/login" class="text-blue-400">Login</a>'}</p></div>`;
