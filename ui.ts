...
async function load2DHistory() {
    ...
    try {
        ...
        let html = '';
        if (history.length === 0) {
            html = '<div class="p-4 text-center text-slate-500 text-xs">No historical results found.</div>';
        } else {
            ...
            for (const date in grouped) {
                const dateDisplay = new Date(date).toLocaleDateString("en-US", { weekday: 'short', month: 'short', day: 'numeric' });
                
                // Date Header
                html += \`<div class="bg-slate-700/50 text-slate-200 px-4 py-2 text-xs font-bold uppercase">${dateDisplay}</div>\`;
                
                // Results for the day
                grouped[date].forEach(item => {
                    html += \`<div class="flex justify-between items-center px-4 py-3 border-b border-slate-700 last:border-b-0">
                                <div class="text-xs text-slate-400">${item.time}</div>
                                <div class="font-bold text-3xl text-yellow-400 drop-shadow-md">${item.twod}</div>
                                <div class="text-right text-sm">
                                    <div class="text-slate-300">${item.set}</div>
                                    <div class="text-green-400 text-xs">${item.value}</div>
                                </div>
                              </div>\`;
                });
            }
        ...
        listEl.innerHTML = '<div class="p-4 text-center text-red-400 text-sm">Error loading history.</div>';
...
function updateBetInput() { 
    ...
    if (type === 'double') { 
        ...
        if (type === 'head') { 
            input.placeholder = "0-9 (Head)"; 
            input.maxLength = 1; 
            input.value = ''; 
        } else if (type === 'tail') { 
            input.placeholder = "0-9 (Tail)"; 
            input.maxLength = 1; 
            input.value = ''; 
        } else { 
    ...
    document.querySelectorAll(".api-stock-loader").forEach(async (el) => { 
        ...
        try { 
            ...
            if(text.includes("0") || text === "?") { 
                ...
                if(btn) { 
                    btn.disabled = true; 
                    btn.className = "w-full text-sm bg-slate-700 cursor-not-allowed text-white font-bold py-2 rounded-lg transition flex justify-center items-center gap-2"; 
                    btn.innerText = "🚫 Out of Stock"; 
                    btn.removeAttribute("onclick"); 
                } 
                if(badge) { 
                    badge.className = "text-[10px] px-2 py-1 rounded whitespace-nowrap bg-red-500/20 text-red-400"; 
                } 
            } 
        } catch { el.innerText = "?"; } 
    }); 
...
function copyPurchasedCode() { 
    ...
    // Use navigator.clipboard.writeText for modern browsers, or a fallback if needed
    if (navigator.clipboard) {
        ...
        // Fallback for document.execCommand('copy') not used here for brevity/modern usage focus
    }
} 
...
export const HistoryTable = (transactions: Transaction[], nextCursor: string | null, activeTab: string) => { 
    ...
    if (transactions.length === 0) { 
        ...
        rows = transactions.map(t => { 
            ...
            let sign = ''; 
            let bg = 'bg-slate-700'; 
            if(t.type === 'purchase' || t.type === 'transfer_sent') { 
                color = 'text-red-400'; 
                sign = '-'; 
                bg = 'bg-red-500/20'; 
            } else if (t.type === 'topup' || t.type === 'voucher' || t.type === 'bonus' || t.type === 'transfer_received' || t.type === 'refund' || t.type === 'win_2d') { 
